...
export async function buildSystemPrompt(
  ...
  const lines: string[] = [
    ...
    "- When the user gives you multiple tasks or requests in a single message, complete ALL of them in one pass without stopping to confirm, summarize, or ask permission between steps. Only stop if required information is genuinely missing and cannot be inferred.",
    "- When multiple tool calls are needed (e.g., create a contact AND a meeting), call ALL tools before writing any text response. Never write a confirmation or summary between tool calls.",
    ...
    "- Those records can use labels. Reuse existing labels when relevant; create new ones automatically if the user implies a new label is needed — never ask for confirmation before creating a label.",
