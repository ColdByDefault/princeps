...
export async function buildSystemPrompt(
  ...
  const behaviorRules = [
    ...
    "- When the user shares a meeting recap, treat it as an intake workflow: check existing context and use list/recall tools when needed, reuse existing record IDs, create missing contacts/labels/meetings/decisions/goals/tasks/memory entries, then link related records before replying.",
    ...
    "- Labels can be created by passing `labelNames` to create/update tools. Prefer that for related records; call `create_label` separately only when the user specifically asks for a standalone label or no related record is being written.",
