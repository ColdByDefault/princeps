...
- **Meeting Prep Pack** — `prepPack String?` added to `Meeting` model + migration `20260329155617_add_meeting_prep_pack`, `lib/meetings/prep.logic.ts` (fetches meeting + participants + top-5 knowledge chunks, calls Ollama non-streaming, persists to `Meeting.prepPack`), `app/api/meetings/[id]/prep/route.ts` (GET: return existing; POST: force regenerate), `MeetingList.tsx` upgraded with per-row "Generate prep" button + collapsible prep panel (also Regenerate + "Meeting Prep" badge), `MeetingRecord` type updated with `prepPack` across `types/api.ts` + all logic files, i18n keys `meetings.prep.*` (en + de).
- **Contact Interaction History** — `ContactInteraction` Prisma model + migration `20260329161011_add_contact_interactions`, `lib/contacts/log-interaction.ts` (fire-and-forget helper), `lib/meetings/create.logic.ts` + `update.logic.ts` patched to call `logInteraction` for each participant, `lib/context/contacts.slot.ts` upgraded to inject `lastInteraction` date per contact, `app/api/contacts/[id]/interactions/route.ts` (GET, ownership-checked, last 10 rows), `ContactList.tsx` upgraded with expandable rows showing interaction timeline (chevron toggle, source badge, date), i18n keys `contacts.interactions.*` + `contacts.editLabel/deleteLabel` (en + de).
- **Proactive Nudges** — `lib/nudges/evaluate.logic.ts` (checks meetings starting within 2 hours with no prep pack, tasks overdue > 1 day, decisions open > 14 days; 24-hour per-item cooldown stored in `User.preferences.nudgeLastFired`), `app/api/nudges/run/route.ts` (POST, authenticated, rate-limited via `nudge_run` 5-minute HTTP-only cookie; fires evaluation in background), `components/nudges/NudgeTrigger.tsx` (client component, fire-and-forget mount call), wired into `app/layout.tsx` alongside `ChatWidgetProvider`.
- All checks passing: lint ✅ typecheck ✅ build ✅ (43 routes).
