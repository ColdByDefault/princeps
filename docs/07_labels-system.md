...
Labels are global, user-scoped tags that can be attached to any record in See-Sweet (tasks, contacts, meetings, decisions, knowledge documents). They are created and managed in **Settings → Labels** and are available in every feature that supports them.
